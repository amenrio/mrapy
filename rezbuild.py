...
def build(source_path, build_path, install_path, targets):
    ...
    def _build():
        ...
        # binaries
        mode = stat.S_IRUSR | stat.S_IRGRP | stat.S_IXUSR | stat.S_IXGRP

        if not os.path.exists(os.path.join(source_path,"bin")):
            return
    ...
    def _install():
        for name in ("bin", "src"):
            if not os.path.exists(os.path.join(build_path, name)):
                continue
